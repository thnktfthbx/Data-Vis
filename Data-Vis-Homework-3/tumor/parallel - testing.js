...
// Box Plot canvas
boxplotspace = document.getElementById('boxplotspace').getContext('2d');
boxplotspace.strokeStyle = "rgba(0,100,160,0.1)";
...
/////////////////////////////////////////////////////////////////////
function makeBoxs(selections, ctx) {
  //if (color) ctx.strokeStyle = color;
  stats = calcStats(selections);
  dimensions.map(function(p,i) {
		var cWid = 10;
		var cHit = yscale[p](stats[p]["Q3"]) - yscale[p](stats[p]["Q1"]);
		var x = xscale(p)-cWid/2;
		var y = yscale[p](stats[p]["Q1"]);	
		var verLen = yscale[p](stats[p]["nmax"])- yscale[p](stats[p]["nmin"]);
	    ctx.strokeStyle="black";
		ctx.lineWidth = 1.2;
		ctx.fillStyle="#ffff66";
						  
	    ctx.beginPath();
		ctx.rect(x, yscale[p](stats[p]["nmin"]), cWid, 1);  //min horizontal line
		ctx.rect(x, yscale[p](stats[p]["nmax"]), cWid, 1);  //max horizontal line
		ctx.rect(xscale(p)-1, yscale[p](stats[p]["nmin"]), 1, verLen);  //vertical line
		ctx.stroke();
		
		ctx.beginPath();
		ctx.rect(x, y, cWid, cHit);  //Box
		ctx.rect(x, yscale[p](stats[p]["med"]), cWid, 1);  //med horizontal line
		ctx.fill();
		ctx.stroke();
  });                             // right edge
...
function calcStats(selections) {
	var stats = [];
	for (i = 0; i < dimensions.length; i++) {
		var selectedNums = [];
		for (j = 0; j < selections.length; j++) {
			selectedNums.push(selections[j][dimensions[i]]);
		}
		calNums = calcQuartiles(selectedNums);
		var temp = {nmax: Math.max.apply(Math, selectedNums),
				nmin: Math.min.apply(Math, selectedNums),
				Q1: calNums["Q1"],
				med: calNums["Q2"],
				Q3: calNums["Q3"]};
		stats[dimensions[i]] = temp;
	}
	return stats;	
};

function calcQuartiles(values) {
	//HACK MATH LOGIC
...
function paths(selected, ctx, count) {
  ...
  boxplotspace.clearRect(0,0,w+1,h+1);
...
/////////////////////////////////////////////////
  makeBoxs(shuffled_data, boxplotspace);
///////////////////////////////////////////////////
  d3.timer(animloop);
}
